...
for x in dat:            #print(x['id'],x['api_method'],x['api_url'],x['body'],x['resource_headers'])
	my_dict = {'id' : x['id'],'method':x['api_method'],'url' : x['api_url'], 'body' : x['body'],'resource_name' : x['resource_name'],'resource_headers' : x['resource_headers']}	#print(my_dict['resource_name'])   #print(type(my_dict))
...
for dict_data in my_list:
	...
	if dict_data['method'] == "GET":
		...
		print('URL  =' +  my_url)
		print('BODY =' +  dict_data['body'])
		print('RESOURCE_NAME  =' +  dict_data['resource_name'])
