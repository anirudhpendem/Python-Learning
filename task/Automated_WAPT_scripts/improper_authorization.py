...
try:
    ...
    for url in admin_urls:
        final_response = requests.post(url, data =user_form_data, allow_redirects=True)
        response_body = final_response.content.decode("utf-8")
        try:
            request_body = json.loads(final_response.request.body)
        except:
        ...
        if (final_response.status_code == 200) and save == "True":
            ...
            final_list.append({"Url":url,"Status_code":final_response.status_code,"Status":status,"Request_headers":dict(final_response.request.headers),"Response_headers":dict(final_response.headers),"Timestamp":timestamp})
        else:
            final_list.append({"Url":url,"Status_code":final_response.status_code,"Status":"False","Request_headers":dict(final_response.request.headers),"Response_headers":dict(final_response.headers),"Timestamp":timestamp})
